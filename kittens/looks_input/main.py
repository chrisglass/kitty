...
favorites_path = os.path.join(config_dir, 'looks-input-favorites.conf')
INDEX_CHAR = '.'
    ...
    def layout(self, rows, cols):
        ...
        if self.mode is NAME:
            def as_parts(i, codepoint):
                ...
                return encode_hint(i).ljust(idx_size), " ", name(codepoint)
...
class LooksInput(Handler):

    def __init__(delf, cached_values):
        self.cached_values = cached_values
        self.selected_look = None  # We'll put the look the user selected in here.
        self.choice_line = ''
        self.mode = globals().get(cached_values.get('mode', 'NAME'), 'NAME')
        self.table = Table(self.emoji_variation) # the table painter.
        self.line_edit = LineEdit()

    """
    Read the edit_line.current_input and update self.selected_look with the actual look.
    """
    def update_current_look(self):
        #TODO: Implement
        pass


    def update_prompt(self):
        self.update_current_look()
    ...
    def draw_screen(self):
        ...
        if self.mode is NAME:
            writeln(_('Enter words from the name of the look'))
        else:
    ...
    # Event handlers from handler class below:
...
class UnicodeInput(Handler):
    ...
    def init_terminal_state(self):
        ...
        self.write(set_window_title(_('looks.wtf input')))
    ...
    def draw_screen(self):
        ...
        if self.mode is NAME:
            writeln(_('Enter words from the name of the look'))
        elif self.mode is TAG:
            writeln(_('Enter words from the tag of the look'))
        else:
            writeln(_('Enter the index for the character you want from the list below'))
        self.line_edit.write(self.write, self.prompt)
        with cursor(self.write):
...
help_text = 'Input a look from looks.wtf'
usage = ''
OPTIONS = '''
--emoji-variation
type=choices
...
def parse_unicode_input_args(args):
    return parse_args(args, OPTIONS, usage, help_text, 'kitty +kitten looks_input')
...
def main(args):
    ...
    loop = Loop()
    with cached_values_for('looks-input') as cached_values:
